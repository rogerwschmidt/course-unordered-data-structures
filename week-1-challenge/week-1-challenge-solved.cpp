...
int insert(int value, std::vector<int> &table) {
  ...
  int hashedIndex = hash(value, table.size());
  int insertOffset = 0;

  while(table[hashedIndex + insertOffset] != -1 && hashedIndex + insertOffset <= table.size()){
    insertOffset++;
  }

  table[hashedIndex+insertOffset] = value;
  return insertOffset;
}
