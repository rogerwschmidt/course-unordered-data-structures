...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  // for all the original neighbors, look up their index for all other vertices
  // and erase themselves
  for (const auto& neighborIndex : originalNeighbors){
    adjacencyMap.at(neighborIndex).erase(p1);
  }
